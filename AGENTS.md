...
> 本文档约束 AI Agent 在本项目中的协作行为。所有 Agent 在开始工作前必须阅读并遵守。
>
> 项目阶段：MVP 0.9.4 | 技术栈：Next.js 14 + TypeScript + shadcn/ui + Prisma + SQLite + DeepSeek

> 2026-06-01 协作补充：AI 报告生成与展示必须记录并展示生成时间；专项分析新数据判定以 `sourceMeta` 与数据库来源字段为准；AI 提示词应结合 WHO/CDC 青少年身体活动建议、NSCA 青少年训练原则和项目反标签化要求，不得推测或补全缺失体测项目；单项专项报告必须拆分为「专项动作训练指导」与「阶段训练规划指导」：动作指导只写动作教学卡（动作步骤、训练量/时长、训练要点、常见错误、进阶/降阶、自测标准），不得写第几周；阶段规划才写第几周、阶段目标、动作组合和训练版本。
...
- ❌ 不得基于不完整数据生成完整体质评价（缺失项目必须标注"暂无数据"）
- ❌ 不得用默认值/平均值/班级均值填充缺失项目成绩
- ❌ 不得在 AI prompt 中允许推断或补全未录入体测项目
- ❌ 不得在页面进入时自动触发 AI 报告生成（必须由用户主动点击）
- ❌ 已有报告时不得自动重新生成，须用户确认
- ❌ 不得在 Git 提交中包含 .env.local、prisma/dev.db、node_modules、.next
...
### AI 分析策略（已决策）
- **batch_report 正式体测分析**：全维度画像报告（13 模块）— 数据来源摘要 → 一句话结论 → 完整度 → 总体画像摘要 → 六维体质画像 → 各项目成绩表 → 项目关系分析 → 优势能力分析 → 重点关注项目 → 阶段训练参考 → 教学参考 → 安全提醒 → 教师审核须知
- **item_report 单项分析**：专项能力分析（14 模块）— 报告头部 → 一句话结论 → 数据来源摘要 → 项目科普与评分解读 → 正式体测基线 → 日常训练趋势 → 专项能力拆解 → 影响因素分析 → 问答反馈洞察 → 与其他项目关系 → 专项动作训练指导（动作库）→ 阶段训练规划指导（按阶段/周次组合动作）→ 安全提醒 → 教师审核须知
- **record_report 记录反馈**：针对当前记录的局部分析
- **记录问答系统 v2**：通用体感（训练目的+RPE+恢复+酸痛+不适）+ 专项技术问题，分组展示
- **报告历史**：每次分析保存到数据库，学生可随时回看
- **超时回退**：AI API ≥30s 无响应自动回退 Mock 数据
...
### 双智能体协作
本项目采用 Trae CN 多窗口双智能体模式开发：
- **主窗口**：负责架构设计、核心功能开发、代码审查
- **辅助窗口**：负责功能实现、文档同步、测试验证

### 协作约定
1. 主窗口负责架构决策和代码审查
2. 辅助窗口负责功能实现和文档同步
3. 两个窗口共享项目文档作为统一认知基础
...
> 📋 **本文档目标**：让 AI Agent 在协作开发中保持一致的规范认知，避免各自为政。
>
