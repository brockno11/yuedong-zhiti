...
> 2026-06-01 协作补充：AI 报告生成与展示必须记录并展示生成时间；专项分析新数据判定以 `sourceMeta` 与数据库来源字段为准；AI 提示词应结合 WHO/CDC 青少年身体活动建议、NSCA 青少年训练原则和项目反标签化要求，不得推测或补全缺失体测项目；单项训练建议必须写成可执行的动作教学卡，说明动作步骤、训练要点、常见错误、进阶周期和自测标准。
...
- **item_report 单项分析**：专项能力分析（13 模块）— 报告头部 → 一句话结论 → 数据来源摘要 → 正式体测基线 → 日常训练趋势 → 专项能力拆解 → 影响因素分析 → 问答反馈洞察 → 与其他项目关系 → 专项训练方案 → 短中长期目标 → 安全提醒 → 教师审核须知
