...
本项目在 `.claude/skills/` 目录下安装了 **15 个项目级 Skills**，按优先级分为三层：

| 优先级 | 数量 | 用途 |
|--------|------|------|
| **P0 核心必装** | 6 个 | 项目架构骨架、移动端适配、数据可视化 |
| **P1 强烈建议** | 7 个 | UI 设计、数据处理、项目管理、测试验证、代码审查 |
...
### P1 测试与审查（3 个，v0.9.4 新增）

#### 13. e2e-testing
- **路径**: `.claude/skills/e2e-testing/SKILL.md`
- **来源**: 项目自建
- **主要作用**: Playwright E2E 测试指南 — 环境准备、测试脚本模板、学生端/教师端必测路径、常用断言、响应式检查
- **适合场景**:
  - 代码修改后验证页面功能
  - 全站走查检测 NaN/undefined/溢出
  - 移动端 375px 响应式验证
- **Claude Code 使用方式**: 修改功能后自动触发测试验证
- **Codex 参考方式**: 理解测试路径和断言模式

#### 14. code-audit
- **路径**: `.claude/skills/code-audit/SKILL.md`
- **来源**: 项目自建
- **主要作用**: 系统性代码审查方法论 — 8 个审查维度（类型安全、除零/NaN、React 状态、输入校验、权限检查、数据一致性、UI/UX、安全检查）、优先级定义、审查报告格式
- **适合场景**:
  - 全面代码质量审查
  - 安全审计
  - Bug 系统性排查
- **Claude Code 使用方式**: 用户要求"找 bug"/"代码审查"时自动触发
- **Codex 参考方式**: 理解审查维度和风险模式

#### 15. playwright-integration
- **路径**: `.claude/skills/playwright-integration/SKILL.md`
- **来源**: 项目自建
- **主要作用**: 可直接运行的 Playwright 集成测试脚本 — 一键检测所有学生端/教师端页面的 NaN、溢出、版本号、学段、时间格式、控制台错误
- **适合场景**:
  - 快速全站验证
  - 修改后回归测试
  - 演示前检查
- **Claude Code 使用方式**: 用户要求"跑测试"/"全站检测"时自动触发
- **Codex 参考方式**: 复用测试脚本模板
