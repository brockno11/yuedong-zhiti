...
import {
  GraduationCap, Building2, Users, LogOut, ShieldCheck, Sparkles, ChevronRight, School,
} from "lucide-react";
import Link from "next/link";

interface LoginData { role: string; username: string; name: string; class?: string; timestamp: number; }
...
export function TeacherProfileClient() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5">
      <Card className="rounded-xl border shadow-sm">
        <CardHeader className="pb-3">
          <div className="flex items-center gap-4">
            ...
            <div>
              <CardTitle className="text-base">{loginData?.name || "教师"}</CardTitle>
              <p className="text-sm text-muted-foreground">{loginData?.class || ""} · 体育教研组</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
        ...
        <Link href="/teacher">
          <Card className="cursor-pointer rounded-xl border shadow-sm transition-shadow hover:shadow-md">
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div className="min-w-0 flex-1"><p className="text-sm font-semibold">班级总览</p><p className="text-xs text-muted-foreground">查看体测数据概况</p></div>
        ...
        <Link href="/teacher/review">
          <Card className="cursor-pointer rounded-xl border shadow-sm transition-shadow hover:shadow-md">
            <CardContent className="flex items-center gap-3 p-4">
              ...
              <div className="min-w-0 flex-1"><p className="text-sm font-semibold">审核中心</p><p className="text-xs text-muted-foreground">AI 报告待审核</p></div>
              <ChevronRight className="h-4 w-4 text-muted-foreground" />
            </CardContent>
          </Card>
        </Link>
      ...
      <Card className="rounded-xl border shadow-sm">
        <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-1.5"><Sparkles className="h-4 w-4 text-primary" />AI 使用说明</CardTitle></CardHeader>
      ...
      <Button variant="outline" onClick={handleLogout} className="w-full h-11 gap-2 text-muted-foreground">
