...
- 单项分析训练建议升级为”动作教学卡”：每个动作要求输出训练目的、动作步骤、训练要点、常见错误、强度、进阶方式、自测标准和周期建议；旧报告缺少字段时前端自动兜底展示通用训练指导。
- **新增”项目科普与评分解读”模块**：在 AI 报告详情页中嵌入基于《国家学生体质健康标准（2014年修订）》的本地静态科普板块。评分规则、权重、项目意义全部来自 `src/lib/fitness-education.ts` 静态配置，不依赖 AI 生成，避免 AI 编错国家标准。专项报告显示 `ItemEducationCard`（含可视化评分尺 + 成绩定位点），总体报告显示 `StandardEducationOverview`（含六维能力说明 + 权重条形图 + 缺失项目列表）。AI 提示词中明确要求不重复编写国家标准科普、不推测缺失项目。
- **修复专项分析训练结构**：`专项动作训练指导` 固定为动作教学库，只展示动作怎么做、训练量/时长/强度、要点、常见错误、进阶/降阶与自测标准；`阶段训练规划指导` 才展示第几周、阶段目标、每周频率、动作组合和不同训练版本。旧版 `trainingPlan` 会自动拆解为动作库卡片，阶段/周次只进入阶段规划区。
- **修复专项报告关键 UI 与数据兜底**：历史报告打开时自动推断 `itemId` 并补正式体测基线，保证"项目科普与评分解读"始终显示本次成绩、标准分、等级和评分尺；AI 超时/Mock 回退按 `targetItemId` 生成项目匹配的本地回退报告，避免不同项目正文串台。
- **修复重新生成流程与列表编号**：重新生成报告不再先删除旧报告并 `router.refresh()`，而是在当前详情内进入生成进度，完成后回到同一专项详情；教师审核须知/安全提醒由前端统一编号，并清洗 AI 文本自带编号，避免 `1. 1.`。
- **优化正式体测标准导读**：`身体形态` 显示为 `BMI参考维度`，不再误标"暂无数据"。
- POST `/api/fitness-records` 成功后调用 `revalidatePath`，确保 `/ai-guide`、`/portrait`、`/dashboard` 页面获取最新数据。
- 修复 `useAuthGuard` 中 `router` 引用不稳定导致导航卡死的问题，移除 useEffect 依赖数组中的 `router`。
...
- **教师**：1 位体育教师（张老师，账号 `zhoulaoshi`，密码 `demo123`）
...
| 6 | `/ai-reports` | Server | AI 报告历史列表：分类 Tab（全部/正式分析/专项/反馈）+ 单选/批量删除 + 展开详情 | 无Tab |
...
单项目记录（1 项） → 专项分析模式 ⚡
  ├── 深入分析该项目的技术水平
  ├── 展示项目科普与评分解读（含成绩、标准分、等级、评分尺）
  ├── 提供专项动作训练指导（动作教学库，不写第几周）
  ├── 提供阶段训练规划指导（按阶段/周次组合动作库）
  └── 评估该项目与其他体能维度的关联
...
6. **本阶段训练重点**：AI 指导首页只展示轻量阶段重点和优先项目，不展示复杂周计划
7. **训练观察**：近 7/30 天训练次数、最近训练、恢复提示，并明确"日常训练不参与正式体测评分"
8. **历史分析**：底部折叠列表，区分报告类型、生成时间和审核状态
9. **ReportContent**：报告详情包含项目科普评分、动作教学库、阶段训练规划、恢复与安全提醒、教师审核状态
...
- 重新生成报告：停留在当前详情内进入生成进度，不删除旧报告、不刷新回首页；生成结果完成后回到同一报告类型/同一项目
- item_report 回退：服务端与前端都按 `targetItemId` 构建项目匹配的本地回退报告，防止 Mock 报告与当前项目不一致
...
**内容区域**：
1. **基础信息卡片**：头像占位 + 姓名 + 年级/性别/年龄
2. **基础身体数据**：可编辑身高/体重（120-220cm / 25-150kg），自动计算 BMI，温和表达提示。学生自填数据，标注「用于训练建议参考，不替代正式体测数据」
3. **体测与训练概况**：正式体测次数/最近日期 + 日常训练次数/最近日期 + AI 报告数量
4. **运动目标与基础**：双列卡片
5. **健康关注信息**：标签展示 + 隐私提示
6. **历史记录入口**：显示记录次数，卡片样式可点击
7. **隐私说明**：3 条数据保护说明
8. **AI 使用说明**：4 条规范说明
...
**功能**：
- Tab 切换：待审核 / 已处理
- **批量审批**（v0.9.3）：每张审核卡片左侧 checkbox，支持全选/单选，顶部「批量通过」和「批量退回」按钮
  - 批量操作需二次确认弹窗：「AI 内容仍需教师负责审核」
  - 批量退回需填写退回原因
  - 操作后列表自动刷新，选择清空，成功/失败反馈
  - 仅 pending 状态报告可批处理，非法状态提交被拒绝
...
2. `TeacherReview` 表：创建/更新对应的待审核记录（reviewer: "张老师"）
3. 审核中心以数据库为唯一权威来源
...
│   └── floating-back-bar.tsx — AI 报告详情悬浮返回栏：返回按钮 + 当前报告动态标题
│
├── forms/       # 表单组件
...
└── features/    # 业务组件
    ...
    │   ├── ai-report-detail.tsx     — 报告详情：batch_report 13模块 + item_report 深度详情；动作教学库与阶段规划分区展示
    │   └── ai-generation-status.tsx — 生成状态：分步进度条+骨架屏+错误/回退态
    ├── ai-guide-page-shell.tsx    — AI 指导页标题显隐外壳：详情页隐藏大标题
    ├── item-education-card.tsx    — 专项项目科普与评分解读：成绩/标准分/等级/评分尺/项目说明
    ├── standard-education-overview.tsx — 正式体测标准导读：六类能力、BMI参考维度、权重构成
...
| 模块 | 完成度 | 说明 |
|------|--------|------|
| 项目初始化 + 配置 | 100% | Next.js 14 + shadcn/ui 22 组件 + Tailwind |
| Prisma 数据模型（6 表） | 100% | Schema + migrations + seed 脚本 |
| SQLite 数据持久化 | 100% | 读写正常，dev.db 本地文件 |
| 演示登录系统 | 100% | 角色切换+账号下拉+密码+AuthGuard |
| 学生端 6 页面 | 100% | 新增 /ai-reports AI 报告历史列表 |
| 教师端 6 页面 | 100% | 审核流可用 + 批量审批 |
| 14 个 API 端点 | 100% | 全部通过测试 |
| AI API（Mock + DeepSeek） | 100% | 双模式切换+超时回退+报告持久化 |
| AI 分析策略（专项/综合） | 100% | 单项目专项分析 + 多项目综合分析 |
| AI 报告历史 | 100% | 数据库存储+前端列表+分类筛选+删除 |
| 逐项体感采集 | 100% | 每个项目独立疲劳/恢复/酸痛反馈 |
| 4 种图表 | 100% | 雷达图+趋势图+柱状图+环形图 |
| iOS 液态玻璃导航 | 100% | 响应式+安全区+桌面端适配 |
| 个人中心身体数据编辑 | 100% | 身高/体重可编辑+BMI温和展示+PATCH API |
| 教师批量审批 | 100% | 多选+批量通过/退回/删除+二次确认 |
| 批量删除（学生端） | 100% | AI 报告+日常训练（正式体测不可删） |
| 教师姓名统一 | 100% | 全项目 displayName 统一为"张老师" |
| 死代码清理 | 100% | 删除 7 个未使用文件，种子数据整合 |
...
>
> **最后更新**：2026-06-01 · 版本 MVP 0.9.4 · AI 专项报告动作库/阶段规划分层修复
