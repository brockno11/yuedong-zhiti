...
#### 1. 🏠 体质画像仪表盘 `/dashboard`
- **设计布局**：移动端单列 → 桌面端 5 列网格（雷达图占 3 列 + 趋势图占 2 列）
- **核心指标卡片**：综合评分（大字 4xl）、BMI 状态、最近学期
- **体质雷达图**：5 维（速度/力量/耐力/柔韧/身体形态），个人 vs 班级均值双线
- **成绩趋势图**：50米跑历史趋势折线图（支持多学期对比）
- **本期洞察**：AI 风格智能评语 + 优劣项目标签
- **优势/待提升项目**：颜色标记（绿=优势、橙=待提升）
- **AI 指导入口卡片**：引导进入 `/ai-guide`
- **空状态处理**：无记录时显示引导入口链接到 `/record`

#### 2. 📝 引导式信息填写 `/onboarding`
- **5 步卡片式流程**（非传统长表单）：
