...
#### 4. 🤖 AI 智能指导 `/ai-guide`（v0.5.2 报告中心模式）
- **进入不自动生成**：打开页面不消耗 token，仅读取已有报告和体测记录
- **按记录生成**：基于 sourceRecordId 匹配报告，一份记录对应一份报告
- **状态机驱动**：无记录/无报告/有报告/有新记录/查看历史 — 不同状态显示不同 UI
- **报告生成确认**：重新生成需二次确认，防止误操作消耗 token
- **报告内容结构**（从上到下）：
  - 📊 **报告状态卡**：当前报告/可生成/发现新记录
  - 🧠 **首屏摘要**：审核状态 + 一句话体质画像 + 本周优先目标
  - 📋 **数据完整度**：局部分析（<70%）/综合分析（≥70%）+ 已录/待补充列表
  - 🎯 **重点提升**：最多3项，含当前表现、原因、改进方向
  - 💪 **本周训练计划**：按周展示，每个动作名称/频率/时长/注意事项
  - 🛡️ **恢复与安全提醒**
  - ✅ **教师审核状态**：待审核/已审核/已退回 — 不同文案
  - 📚 **历史报告**：底部 Accordion 默认折叠，切换查看不触发 AI 生成
