...
#### 4. 🤖 AI 智能指导 `/ai-guide`（v0.9.2 全维度报告中心）

AI 指导页已升级为**「AI 报告中心」**，四区域清晰布局，正式体测报告为 10 板块全维度分析：

- **四大区域**：正式体测分析 → 单项分析（6 项目卡片）→ 训练情况 → 历史报告（折叠/展开）
- **正式体测全维度报告**（10 个板块）：
  1. 报告总览（摘要/评分/等级）
  2. 各项目成绩表（6 项，缺失显示"暂无数据"）
  3. 六维度表现（身体形态/心肺耐力/速度/爆发力/柔韧/肌肉力量，AI 未返回时前端兜底计算）
  4. 项目关系分析（速度+爆发力、心肺+耐力、柔韧+跑步）
  5. 重点关注项目（含优先级、原因、训练建议）
  6. 优势与提升空间
  7. 阶段训练参考（3 阶段可折叠，适应→强化→巩固）
  8. 教学参考（给体育教师的课堂指导）
  9. 恢复与安全提醒（5-6 条）
  10. 审核状态说明
- **批次切换**：支持多正式体测批次，通过 sourceRecordId→batchId 精确匹配报告
- **数据预览**：批次无 AI 报告时基于实际记录自动计算预览，含"生成 AI 分析"按钮
- **完整度按批次算**：只统计最新批次内项目，不跨批次混合
- **recordType 严格筛选**：`official_test` 正向匹配，不依赖排除法
- **analysisScope**：API 解析分析范围注入提示词上下文
- **底部导航返回**：Tab re-click 可重置报告详情回到报告中心
- **三类 AI 报告**：`batch_report`（全维度正式体测）、`item_report`（专项分析）、`record_report`（本次反馈）
...
- **运动目标与基础**：双列卡片
- **健康关注信息**：标签展示 + 隐私说明
- **历史记录入口**：显示记录次数，链接到 `/records`（历史记录页，按类型筛选）
- **教师工作台入口**：学生端快捷访问教师端
- **隐私说明** + **AI 使用说明**：4 条说明
...
│   │       ├── ai-student-report.tsx    # AI 报告中心主控（v0.9.2 全维度报告）
