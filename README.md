...
#### 4. 🤖 AI 运动指导中心 `/ai-guide`（v0.7.0 指导中心模式）

AI 指导页已升级为"AI 运动指导中心"，不再是单份长报告：

- 🧠 **整体体质分析卡**（顶部）：数据不足时提示补充，完整度≥70%时可生成综合体质画像
- 🎯 **专项提升卡片网格**（中部）：6 个性别适配项目卡片，每张 4 种状态（未记录/已记录/有报告/已审核）
- 📋 **本次记录分析**：多项目记录时一键生成 record_report
- 📚 **历史报告**（底部折叠）：区分单项专项/本次记录/综合体质三类

**三类 AI 报告**：
- `item_report` 单项专项报告：深入分析一个项目 + 专项训练计划
- `record_report` 本次记录分析：分析已录项目 + 待补充提醒
- `batch_report` 综合体质画像：仅完整度≥70%时生成，五维综合评价
