...
> 2026-06-01 更新：AI 报告详情已显示本次生成时间；专项分析新数据判定已与数据库来源元信息对齐；AI 分析提示词补充 WHO/CDC 与 NSCA 青少年训练原则，并加强反标签化与缺失数据约束；单项训练建议升级为动作教学卡，展示步骤、要点、常见错误、进阶周期和自测标准。
...
- **单项深度分析报告**（item_report，13 个模块）：
  ...
  10. 专项训练方案（2-4 个动作，适合校园或家庭；每个动作含训练目的、动作步骤、训练要点、常见错误、强度、进阶方式、自测标准和周期建议）
  11. 短中长进阶目标（1-2周/3-6周/6-12周）
  12. 项目专项安全提醒（不同项目不同提醒）
  13. 教师审核须知
